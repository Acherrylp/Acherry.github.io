...
gapApp.controller('DashboardCtrl', function($scope, $http, uiGmapIsReady, $modal){
	$scope.charts = {
...
gapApp.controller('ExportCtrl', function($scope, $http, $window){
	$scope.table = '';
	$scope.tables = [
		{name:'Farmer', alias:'รายงานเกษตรกร'},
		{name:'Site', alias:'รายงานแปลงเพาะปลูก'},
		{name:'Product', alias:'รายงานการตรวจประเมิน'}
	];
	$scope.export = function(table){
		if(table == 'Farmer' || table == 'Site' || table == 'Product'){
			$window.location.href = 'api/export/'+table+'.csv';
		}
/* 		$window.location.href = '/api/export/'+$scope.table; */
/* 		return $http({
				method: 'GET',
				url: 'api/export/'+$scope.entity,
				params: $scope.input
			}).success(function(response){
				$scope.sites = response;
			});	 */
