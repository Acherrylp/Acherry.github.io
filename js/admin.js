...
gapApp.config(function($routeProvider, uiSelectConfig, uiGmapGoogleMapApiProvider){
	$routeProvider.when('/dashboard', {
	 	templateUrl: 'tmpl/dashboard.php',
	...
	$routeProvider.when('/search', {
	 	templateUrl: 'tmpl/search_edit.html',
...
gapApp.controller('DashboardCtrl', function($scope, $http, uiGmapIsReady, uiGmapGoogleMapApi, $modal){
	...
	$scope.fullmap = function(){
		//alert("TEST");
		var win = window.open('tmpl/map.html', '_blank');
  		win.focus();
	};
...
gapApp.controller('SearchCtrl', function($scope, $http, $locale, $modal){
	...
	$scope.map_call = function (lat,lng) {
		alert(lat+" "+lng);
	};
...
/* 	$http.get('api/productStatuses').success(function(response){
	...
	$scope.modeChanged = function(){
		...
		delete $scope.input.person_id;
		delete $scope.input.site_name;
		delete $scope.input.cert_id;
/* 		$scope.$apply(); */
	};
...
gapApp.controller('FarmersCtrl', function($scope, $http, $filter, $locale, $modal){
	$locale.id = 'th-th';window.scope = $scope;
	$scope.page = 1;
	$scope.items = [];
	$scope.farmers = {
		items: null,
...
gapApp.controller('ApiCtrl', function($scope, $http, $modal){
	...
	$scope.params = '';
	$scope.apis = [
		{entity: 'farmer', method: 'index', alias: 'รายการเกษตรกร', url: 'api/farmers', fields:[]},
		{entity: 'site', method: 'index', alias: 'รายการแปลงเพาะปลูก', url: 'api/sites', fields:[]},
		{entity: 'product', method: 'index', alias: 'รายการการตรวจประเมิน', url: 'api/products', fields:[]},
		{entity: 'address', method: 'index', alias: 'ค้นหารหัสที่อยู่', url: 'api/addresses', fields:[]},
		{entity: 'farmer', method: 'show', alias: 'ช้อมูลเกษตรกร', url: 'api/farmer/', fields:[]},
		{entity: 'site', method: 'show', alias: 'ข้อมูลแปลงเพาะปลูก', url: 'api/site/', fields:[]},
		{entity: 'product', method: 'show', alias: 'ข้อมูลการตรวจประเมิน', url: 'api/product/', fields:[]},
		{entity: 'address', method: 'show', alias: 'ช้อมูลที่อยู่', url: 'api/address/', fields:[]}
	];
	$scope.api = {
		input: {},
		selected: $scope.apis[0],
/* 		fields: $scope.apis[0].fields, */
		request: function(){
/* 			$scope.buildParams(); */
		...
		window.scope = $scope;
	$scope.buildParams = function(modeChanged){
		if(modeChanged){
			$scope.api.input.sort = '' ;
		}
		var params = [];
...
gapApp.controller('ExportCtrl', function($scope, $http, $window){
	$scope.table = '';

	// ศูนย์เห็นแค่ Farmer, Area
	// ของกรม เห็นทุกอย่าง
	$scope.tables = [];
	$http({
		method: 'POST',
		url: '../app/export/export.php',
		headers: {'Content-Type': 'application/x-www-form-urlencoded'},
		data: $.param({tables: true})
	...
	$scope.export = function(table){
		var i = 0;
		while(i < $scope.tables.length && table != $scope.tables[i].name) i++;
		if(i > $scope.tables.length ) return false;
		var t = $scope.tables[i];

		$scope.dataLoading = true;
		window.open('../app/export/export.php?report=' + t.name + "&format=xlsx", "_blank");
		$scope.dataLoading = false;
		return true;
	};
});
